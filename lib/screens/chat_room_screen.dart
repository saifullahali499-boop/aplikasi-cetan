...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
 ...
 // 👈 BARU: State untuk menampung pesan yang sedang dibalas
 ...
 void _uploadMedia(Uint8List bytes, String fileName, String type) async {
   ...
   try {
     ...
     await _firestore.collection('chats').add({
       ...
       'isRead': false,
       'isStarred': false,
       'replyToText': replyText,     // 👈 BARU
       'replyToSender': replySender, // 👈 BARU
     });

     setState(() { _replyingMessage = null; }); // Reset preview balasan
   } catch (e) {
 ...
 void _sendMessage() async {
   ...
   // Merekam data konteks balasan jika ada
   String? replyText = _replyingMessage != null ? _replyingMessage!['text'] : null;
   String? replySender = _replyingMessage != null ? _replyingMessage!['sender'] : null;
   ...
   try {
     ...
     setState(() { _replyingMessage = null; }); // Reset setelah terkirim
   } catch (e) {
         ...
         child: Column(
           ...
           children: [
             ...
             ListTile(
               ...
               onTap: () {
                 Navigator.pop(context);
                 // 👈 BARU: Set state pesan yang dibalas agar muncul preview di atas textfield
                 setState(() {
                   _replyingMessage = chat;
                 });
               },
             ),
             ListTile(
               ...
               onTap: () {
                 Navigator.pop(context);
                 // 👈 BARU: Memanggil dialog pemilihan ruang obrolan tujuan transfer pesan
 ...
 // 👈 FUNGSI BARU: Dialog Teruskan untuk menduplikasi pesan ke Room lain secara real-time
 void _showForwardDialog(BuildContext context, Map<String, dynamic> originalChat) {
   showDialog(
     context: context,
     builder: (context) {
       // Daftar list static target room atau kamu bisa query dari Firestore room list milikmu
             ...
             itemBuilder: (context, index) {
               ...
               return ListTile(
                 ...
                 onTap: () async {
                   ...
                   try {
                     // Salin data pesan asli lalu kirim ke room yang dipilih pengguna
                     await _firestore.collection('chats').add({
                       ...
                       'senderUid': currentUser.uid,
                       'timestamp': FieldValue.serverTimestamp(),
                       'room': roomName, // Room baru berganti ke target pilihan
                       'isRead': false,
                       'isStarred': false,
                     });
     ...
     child: Scaffold(
       ...
       body: Column(
         children: [
           ...
           // 👈 BARU: Komponen Preview Balasan di atas kolom ketik pesan (Hanya muncul jika _replyingMessage aktif)
               ...
               child: Row(
                 children: [
                   ...
                   IconButton(
                     ...
                     onPressed: () {
                       setState(() { _replyingMessage = null; }); // Tombol cancel balas
                     },
                   )
                 ],
               ),
 ...
 Widget _buildBubble(BuildContext context, Map<String, dynamic> chat, String time, String messageId) {
   ...
   // Mendapatkan data referensi balasan dari Firestore
           ...
           child: Container(
             ...
             child: Column(
               ...
               children: [
                 ...
                 // 👈 BARU: Struktur tampilan kotak kutipan balasan di dalam balon chat
                   ...
                   Container(
                     ...
                     child: Column(
                       ...
                       children: [
                         Text(
                           replyToSender,
                           style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Color(0xFF533B00)),
                         ),
                         Text(
                           replyToText,
                           maxLines: 1,
                 ...
                 Row(
                   ...
                   children: [
                     ...
                     if (isStarred) ...[
                       Icon(Icons.star, size: 11, color: Colors.grey[700]),
                       const SizedBox(width: 4),
                     ],
                     Text(
                       time,
